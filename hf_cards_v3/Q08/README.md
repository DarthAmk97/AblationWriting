...
- szyszy/GEN
model-index:
- name: AblationWriting-H2-Q08-cone
  results:
  - task:
...
# Qwen3.5-0.8B that writes simpler (H2 safety-cone ablation)

Take a 0.8B Qwen, find the eight activation directions tied to its safety-refusal voice, and shave them off at full strength as it writes. The honest null of the panel: the geometry does nothing here and plain word deletion wins instead. Nothing is retrained and no weights change. Pipeline codename Q08: internal shorthand from the eight-model sweep, safe to ignore.

Words we keep using: rank is how many directions get cut, here 8 out of 2048. Alpha is cut strength, here 1.0, full removal of the positive projection. MMD is distance between two piles of text; smaller against human is better. Recovery is the fraction of the baseline-to-human gap closed. JMQ is a blind taste test judged by another model where 0.50 is a coin flip.

## Abstract

The cone effect on MMD is indistinguishable from zero at +0.020 with CI crossing zero, random ablation hurts at -0.134, and plain lexical deletion wins outright at +0.182. Blind quality preference is parity at 0.472. No GGUF by design, see Usage.

Collection: [AblationWriting](https://huggingface.co/collections/amkkk/ablationwriting-6a9e1c709941b6e3fd1c1dea). Full dump: [AblationWriting-H2-dump](https://huggingface.co/amkkk/AblationWriting-H2-dump). Code and paper: [DarthAmk97/AblationWriting](https://github.com/DarthAmk97/AblationWriting). Siblings: L1-cone, O1-cone, Q20-cone, G2-cone in the same collection.
...
> [!NOTE]
> Parent: [Qwen/Qwen3.5-0.8B](https://huggingface.co/Qwen/Qwen3.5-0.8B). Refs: [Panickssery et al, arXiv:2404.13076](https://arxiv.org/abs/2404.13076), [Joad et al, arXiv:2602.02132](https://arxiv.org/abs/2602.02132). Embedder: [nvidia/llama-embed-nemotron-8b](https://huggingface.co/nvidia/llama-embed-nemotron-8b). Human refs: [szyszy/GEN](https://huggingface.co/datasets/szyszy/GEN).

## Method

Hook: h minus alpha times B times the positive part of B-transpose times (h minus mu), generated tokens only, BF16. Basis B is orthonormal native-space, rebuilt from full_layer npz files plus frozen.json rank and rho. LEX arm is a text-level deletion rule, K token ids at rate q, hash-decided per position, with LEX-MATCH outputs in this repo for comparison. On this model the lexical rule is the one that works.

| Parameter | Value |
|---|---|
| base model | Qwen/Qwen3.5-0.8B, rev 2fc06364 |
...
tok, model, layer_map = load_target('Q08')  # pinned base and revision, BF16
model.eval()
geom = Path('AblationWriting-H2-Q08-cone')  # this repo
record = json.load(open(geom / 'frozen.json'))
source = {'record': record, 'geom': geom, 'model': 'Q08'}
bases = build_control_bases(model, tok, source, layer_map)
alpha = float(record['alpha'])
dev = next(model.parameters()).device
hooks, handles = {}, []
for layer in [18, 19, 20, 21, 22]:
...
The winner on this model. Exact text rule, needs only the eval tokenizer:
...
MODEL, PROMPT_ID = 'Q08', 'test_0001'  # same id plus same baseline text gives the same output, always
...
- Siblings: [L1-cone](https://huggingface.co/amkkk/AblationWriting-H2-L1-cone), [O1-cone](https://huggingface.co/amkkk/AblationWriting-H2-O1-cone), [Q20-cone](https://huggingface.co/amkkk/AblationWriting-H2-Q20-cone), [G2-cone](https://huggingface.co/amkkk/AblationWriting-H2-G2-cone)
- Collection: [AblationWriting](https://huggingface.co/collections/amkkk/ablationwriting-6a9e1c709941b6e3fd1c1dea)
...
|---|---|---|---|
| ANCHOR-CONE | +0.020 | -0.024 to +0.060 | 1.0 |
| RAND-RANK-DOSE | -0.134 | -0.193 to -0.082 | 0.0005 |
| LEX-MATCH | +0.182 | +0.153 to +0.213 | 0.0005, lexical wins |
| Core TEST MMD | +0.022 | point | n/a |
| Blind JMQ | 0.472 ablated, parity | p = 1 | n/a |

## Limitations

- Linear bake impossible: clamp plus generated-only locus cannot fuse into weights, so no GGUF.
- The cone itself is null on MMD here; the interesting result is lexical, not geometric.
- Quality preference is parity, not improvement.
- One deterministic random draw; a second seed would strengthen the rank claim.
- English GEN refs only.

## Scope

Suitable for: reproducing the null honestly, running the lexical winner, comparing lexical vs geometric humanization. Not suitable for: claiming a cone effect on this model, running from this repo without the parent, treating any future GGUF as the measured intervention.

## Provenance and SHAs

- Base rev 2fc06364715b967f1860aea9cf38778875588b17. Frozen run H2-full-s3-Q08-W-A-r8-a1.0-rho0.5-cone.
...
```
@misc{ablationwriting-h2-q08-cone,
  title = {AblationWriting H2 frozen cone: Qwen3.5-0.8B},
