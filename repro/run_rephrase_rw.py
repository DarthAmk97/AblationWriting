...
jobs = json.load(open("rephrase_rw_jobs.json", encoding="utf-8"))
TASK = ("Rephrase the passage below in one single passage. Rules: output ONLY the rephrased passage, "
        ...
        "with Here is, Here are, or any preamble. Keep every number, name, fact, citation key, and LaTeX "
        "command exactly as written. Add no new facts and no new sentences. Passage: ")
outs = []
for j in jobs:
    set_seed(stable_prompt_seed(j["job"] + "-rw"))
    ...
    outs.append({"job": j["job"], "head": j["head"], "out": tok.decode(out[0, n:], skip_special_tokens=True)})
    json.dump(outs, open("rephrase_rw_outs.json", "w", encoding="utf-8"), indent=1)
