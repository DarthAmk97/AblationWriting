...
want = [j for j in jobs if rep.get(j["job"], "").startswith("REJECT") and "STALE" not in rep.get(j["job"], "")]
print("rerun-n", len(want), flush=True)
...
outs = []
for j in want:
    set_seed(stable_prompt_seed(j["job"] + "-v2"))
    ...
    json.dump(outs, open("rephrase_paper_outs2.json", "w", encoding="utf-8"), indent=1)
