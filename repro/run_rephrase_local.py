...
outs = []
for j in jobs:
    set_seed(stable_prompt_seed(j["job"]))
    rendered = tok.apply_chat_template([{"role": "user", "content": "Rephrase this: " + j["text"]}], tokenize=False, add_generation_prompt=True)
    ...
    json.dump(outs, open("rephrase_paper_outs.json", "w", encoding="utf-8"), indent=1)
